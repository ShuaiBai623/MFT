...
for k = 1:length(features)
    ...
    elseif isequal(features{k}.getFeature, @get_colorspace)
        ...
        features{k}.is_cnn = false;
        features{k}.is_drag =false;
    elseif isequal(features{k}.getFeature, @get_cnn_layers_drag)
        ...
        % load the network
        nets{k} = load_cnn_drag(features{k}.fparams, img_sample_sz);
        
        % find the dimensionality of each layer
        % features{k}.fparams.nDim = net.info.dataSize(3, features{k}.fparams.output_layer+1)';
        features{k}.fparams.output_layer_index=nets{k}.getVarIndex(features{k}.fparams.output_layer);

        tmp=nets{k}.getVarSizes({'data',[nets{k}.meta.normalization.imageSize]});
        tmp=cat(1,tmp{:});
        disp(features{k}.fparams.output_layer_index);
        features{k}.fparams.nDim = tmp(features{k}.fparams.output_layer_index,3);
        ...
        %     net_info_stride = [1; 1];
        % end
        net_info_stride = cat(1,nets{k}.getVarReceptiveFields('data').size)';
        % compute the cell size of the layers (takes down-sampling factor
        % into account)
        features{k}.fparams.cell_size = net_info_stride(1, features{k}.fparams.output_layer_index)' .* features{k}.fparams.downsample_factor';
        
        % this feature will always return a cell array
        features{k}.is_cell = true;
        features{k}.is_cnn = true;
        features{k}.is_drag =true;
    elseif isequal(features{k}.getFeature, @get_cnn_layers_drag_py)
        ...
        % load the network
        nets{k} = load_cnn_drag_py(features{k}.fparams, img_sample_sz);
    ...
    elseif isequal(features{k}.getFeature, @get_cnn_layers) || isequal(features{k}.getFeature, @get_OFcnn_layers )
        ...
        % load the network
        nets{k} = load_cnn(features{k}.fparams, img_sample_sz);
        
        % find the dimensionality of each layer
        features{k}.fparams.nDim = nets{k}.info.dataSize(3, features{k}.fparams.output_layer+1)';
        
        % find the stride of the layers
        if isfield(nets{k}.info, 'receptiveFieldStride')
            net_info_stride = cat(2, [1; 1], nets{k}.info.receptiveFieldStride);
        else
            net_info_stride = [1; 1];
        end
...
cnn_output_sz={};
check=1;
if length(cnn_feature_inds) > 0
    % Search for an appropriate image size looking at the last layer
    for i =1:length(cnn_feature_inds)

        cnn_feature_ind=cnn_feature_inds(i);
        if(features{cnn_feature_ind}.is_drag ==false)
            scale = features{cnn_feature_ind}.fparams.input_size_scale;
            
            new_img_sample_sz = img_sample_sz;

            % First try decrease one
            net_info = nets{cnn_feature_ind}.info;
            
            if ~strcmpi(size_mode, 'same') && strcmpi(features{cnn_feature_ind}.fparams.input_size_mode, 'adaptive')
                orig_sz = nets{cnn_feature_ind}.info.dataSize(1:2,end)' / features{cnn_feature_ind}.fparams.downsample_factor(end);
                ...
                while desired_sz(1) > net_info.dataSize(1,end)
                    ...
                    net_info = vl_simplenn_display(nets{cnn_feature_ind}, 'inputSize', [round(scale * new_img_sample_sz), 3 1]);
                end
            ...
            if strcmpi(features{cnn_feature_ind}.fparams.input_size_mode, 'adaptive')
                ...
                features{cnn_feature_ind}.img_input_sz = nets{cnn_feature_ind}.meta.normalization.imageSize(1:2);
            end
            ...
            % disp(size(net_info_stride));
            % disp()
            % disp(features{cnn_feature_ind}.fparams.output_layer+1)
            ...
            cnn_output_sz{cnn_feature_ind} = round(bsxfun(@rdivide, scaled_support_sz, net_stride));
            features{cnn_feature_ind}.fparams.start_ind = floor((total_feat_sz - cnn_output_sz{cnn_feature_ind})/2) + 1;
            features{cnn_feature_ind}.fparams.end_ind = features{cnn_feature_ind}.fparams.start_ind + cnn_output_sz{cnn_feature_ind} - 1;
            
            feature_info.img_support_sz = round(scaled_support_sz .* feature_info.img_sample_sz ./ scaled_sample_sz);
            
            % Set the input size
            features{cnn_feature_ind}.fparams.net = set_cnn_input_size(nets{cnn_feature_ind}, feature_info.img_sample_sz);
            ...
            features{cnn_feature_ind}.fparams.output_layer_index=nets{cnn_feature_ind}.getVarIndex(features{cnn_feature_ind}.fparams.output_layer);
            tmp=nets{cnn_feature_ind}.getVarSizes({'data',[nets{cnn_feature_ind}.meta.normalization.imageSize]});
            tmp=cat(1,tmp{:});
            features{cnn_feature_ind}.fparams.nDim = tmp(features{cnn_feature_ind}.fparams.output_layer_index,3);
            if ~strcmpi(size_mode, 'same') && strcmpi(features{cnn_feature_ind}.fparams.input_size_mode, 'adaptive')
                orig_sz =  tmp(features{cnn_feature_ind}.fparams.output_layer_index(end),1:2)/ features{cnn_feature_ind}.fparams.downsample_factor(end);
                ...
                while desired_sz(1) > tmp(features{cnn_feature_ind}.fparams.output_layer_index(end),1)
                    new_img_sample_sz = new_img_sample_sz + [1, 0];
                    tmp=nets{cnn_feature_ind}.getVarSizes({'data',[round(scale * new_img_sample_sz), 3 1]});
                    tmp=cat(1,tmp{:});
                   
                end
                while desired_sz(2) > tmp(features{cnn_feature_ind}.fparams.output_layer_index(end),2)
                    new_img_sample_sz = new_img_sample_sz + [0, 1];
                    tmp=nets{cnn_feature_ind}.getVarSizes({'data',[round(scale * new_img_sample_sz), 3 1]});
                    tmp=cat(1,tmp{:});
                end
            ...
            %     net_info_stride = [1; 1];
            % end
            net_info_stride = cat(1,nets{cnn_feature_ind}.getVarReceptiveFields('data').size)';
            net_stride = net_info_stride(:, features{cnn_feature_ind}.fparams.output_layer_index)';
            % disp(features{cnn_feature_ind}.fparams.output_layer_index)
            % disp('ssssss')
            % disp(net_stride)
            total_feat_sz = tmp(features{cnn_feature_ind}.fparams.output_layer_index,1:2);
            ...
            cnn_output_sz{cnn_feature_ind} =tmp(features{cnn_feature_ind}.fparams.output_layer_index,1:2);
            ...
            if features{cnn_feature_ind}.fparams.use_gpu
                ...
                features{cnn_feature_ind}.fparams.net.move('gpu')
            end
